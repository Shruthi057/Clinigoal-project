        ...
        try {
          ...
          userCertificates.forEach(cert => {
            ...
            recentCertificates.push({
              ...cert,
              userName: cert.userName || userEmail.split('@')[0],
              userEmail: userEmail
            });
          });
        } catch (error) {
          console.error(`Error processing certificates for ${userEmail}:`, error);
        }
      });
      ...
      <div className="admin-charts-section">
                ...
                datasets: [
                  {
                    ...
                    data: courses.map(course => {
                      ...
                      let count = 0;
                      uniqueUsers.forEach(user => {
                        const userAccessKey = `userCourseAccess_${user.replace(/[@.]/g, '_')}`;
                        const userAccessData = JSON.parse(localStorage.getItem(userAccessKey) || '{}');
                        if (userAccessData[course._id]?.status === 'approved') {
                          count++;
                        }
                      });
                      return count;
        ...
        <div className="admin-chart-card">
          ...
          <div className="admin-chart-container">
            <Doughnut 
              data={{
                labels: ['Active Today', 'Total Users'],
                datasets: [
                  {
                    data: [userStats.todayLogins, userStats.uniqueUsers],
                    backgroundColor: [
                      'rgba(34, 197, 94, 0.7)',
                      'rgba(59, 130, 246, 0.7)'
                    ],
